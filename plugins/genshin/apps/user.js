...
export class user extends plugin {
  constructor (e) {
    super({
      ...
      priority: 300,
      rule: [
        { reg: '^#?(体力|[Cc](oo)?[Kk](ie)?)帮助', fnc: 'ckHelp' },
        { reg: '^#[Cc](oo)?[Kk](ie)?代码$', fnc: 'ckCode' },
        { reg: /^#绑定c(oo)?k(ie)?$/i, fnc: 'bingCk' },
        { reg: '(.*)_MHYUUID(.*)', event: 'message.private', fnc: 'noLogin' },
        { reg: /^#?(原神|星铁|绝区零)?我的c(oo)?k(ie)?$/i, event: 'message', fnc: 'myCk' },
        { reg: /^#?(原神|星铁|绝区零)?删除c(oo)?k(ie)?$/i, fnc: 'delCk' },
        { reg: /^#?(原神|星铁|绝区零)?(删除|解绑)uid(\s|\+)*([0-9]{1,2})?$/i, fnc: 'delUid' },
        { reg: /^#(原神|星铁|绝区零)?绑定(uid)?(\s|\+)*([1-9]|18)[0-9]{7,8}$/i, fnc: 'bingUid' },
        { reg: /^#(原神|星铁|绝区零)?(我的)?(uid)[0-9]{0,2}$/i, fnc: 'showUid' },
        { reg: /^#\\s*(检查|我的)*c(oo)?k(ie)?(状态)*$/i, fnc: 'checkCkStatus' },
        { reg: '^#(接受)?绑定(主|子)?(用户|账户|账号)(\\[[a-zA-Z0-9_\\-:\\]+\\]){0,2}$', fnc: 'bindNoteUser' },
        { reg: '^#(删除绑定|取消绑定|解除绑定|解绑|删除|取消)(主|子)(用户|账户|账号)$', fnc: 'bindNoteUser' }
      ]
    })
