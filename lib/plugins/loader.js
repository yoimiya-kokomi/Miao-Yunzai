  ...
  packageTips (packageErr) {
    ...
    getPlugins() {
        let find = (dir) => {
            ...
            for (let val of fs.readdirSync(dir, { withFileTypes: true })) {
                let filePath = dir + "/" + val.name

                if (val.isFile()) {
                    // js 小插件，可以热更新
                    if (val.name.endsWith('.js')) {
                        let dirPath = dir.replace(/^\.\/plugins\//, "")
                        plug.push({
                            name: dirPath + "/" + val.name,
                            path: "../." + filePath
                        })
                        // 监视，热更新
                        this.watch(dirPath, val.name)
                    }
                    continue
                }

                if (val.isDirectory()) {
                    plug = [...plug, ...find(filePath)]
                }
            }
        ...
        // ./plugins/+ 开始搜寻...
        for (let val of fs.readdirSync("./plugins", { withFileTypes: true })) {
            if (val.isDirectory()) {
                ret = [...ret, ...find("./plugins/" + val.name)]
            }
        }
