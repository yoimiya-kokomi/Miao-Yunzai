...
class PluginsLoader {
  ...
  async getPlugins() {
    let ignore = ["index.js"]
    let files = await fs.readdir(this.dir, { withFileTypes: true })
    let ret = []
    for (let val of files) {
      ...
      let apps = await fs.readdir(`${this.dir}/${val.name}`, { withFileTypes: true })
      for (let app of apps) {
  ...
  count(e, type, msg) {
    if (cfg.bot.msg_type_count)
      for (const i of Array.isArray(msg) ? msg : [msg])
        this.saveCount(e, `${type}:${i?.type || "text"}`)
    this.saveCount(e, `${type}:msg`)
  }
  ...
  watchDir(dirName) {
    ...
    Bot.once("online", () => {
      ...
      watcher.on("add", async PluPath => {
        ...
        if (!await Bot.fsStat(`${this.dir}/${dirName}/${appName}`)) return
