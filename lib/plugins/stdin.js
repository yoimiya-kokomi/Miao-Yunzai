...
Bot.adapter.push(new class stdinAdapter {
  ...
  async sendMsg(msg) {
    ...
    for (let i of msg) {
      ...
      if (i.file)
        i.file = await this.fileType(i.file, i.name)
      ...
      switch (i.type) {
        case "text":
          ...
          break
        case "image":
          logger.info(`${logger.blue(`[${this.id}]`)} 发送图片：${i.file.url}\n文件已保存到：${logger.cyan(i.file.path)}`)
          fs.writeFileSync(i.file.path, i.file.buffer)
          break
        case "record":
          logger.info(`${logger.blue(`[${this.id}]`)} 发送音频：${i.file.url}\n文件已保存到：${logger.cyan(i.file.path)}`)
          fs.writeFileSync(i.file.path, i.file.buffer)
          break
        case "video":
          logger.info(`${logger.blue(`[${this.id}]`)} 发送视频：${i.file.url}\n文件已保存到：${logger.cyan(i.file.path)}`)
          fs.writeFileSync(i.file.path, i.file.buffer)
          break
        case "reply":
          break
