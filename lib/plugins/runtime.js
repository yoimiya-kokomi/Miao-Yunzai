...
export default class Runtime {
   ...
   */
  async getMysApi (targetType = 'all', option = {}, isSr = false, iszzz = false) {
    let mys = await this.getMysInfo(targetType)
    if (mys.uid && mys?.ckInfo?.ck) {
      return new MysApi(mys.uid, mys.ckInfo.ck, option, isSr, iszzz)
    }
    return false
  }
   ...
   */
  async createMysApi (uid, ck, option, isSr = false, iszzz = false) {
    return new MysApi(uid, ck, option, isSr, iszzz)
  }
