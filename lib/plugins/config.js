...
export default async function(name, config = {}, keep = {}, replacer) {
  const configFile = `config/${name}.yaml`
  const configSave = replacer ?
    async () => fs.writeFile(configFile, await replacer(YAML.stringify(config)), "utf8") :
