...
export default class Yunzai extends EventEmitter {
  ...
  wsConnect(req, socket, head) {
    this.wss.handleUpgrade(req, socket, head, conn => {
      ...
      this.makeLog("mark", ["建立连接", req.headers], `${conn.sid} <=> ${conn.rid}`, true)
      conn.on("error", (...args) => this.makeLog("error", args, `${conn.sid} <=> ${conn.rid}`, true))
      conn.on("close", () => this.makeLog("mark", "断开连接", `${conn.sid} <≠> ${conn.rid}`, true))
      conn.on("message", msg => this.makeLog("debug", ["消息", this.String(msg)], `${conn.sid} <= ${conn.rid}`, true))
      conn.sendMsg = msg => {
        if (!Buffer.isBuffer(msg)) msg = this.String(msg)
        this.makeLog("debug", ["消息", msg], `${conn.sid} => ${conn.rid}`, true)
  ...
  async serverEADDRINUSE(err, https) {
    this.makeLog("error", ["监听端口", https ? cfg.bot.https.port : cfg.bot.port, "错误", err], "Server")
    if (https) return
    try {
  ...
  async serverLoad(https) {
    ...
    this[server].listen(https ? cfg.bot.https.port : cfg.bot.port)
    try {
    ...
    this.url = https ? cfg.bot.https.url : cfg.bot.url
  }

  async httpsLoad() {
    try {
      this.httpsServer = (await import("node:https")).createServer({
        key: await fs.readFile(cfg.bot.https.key),
        cert: await fs.readFile(cfg.bot.https.cert),
      }, this.express)
        .on("error", err => {
          if (typeof this[`server${err.code}`] === "function")
  ...
  async run() {
    ...
    if (cfg.bot.https.key && cfg.bot.https.cert)
  ...
  makeLogID(id) {
    if (id === false) return cfg.bot.log_align || "TRSSYz"
