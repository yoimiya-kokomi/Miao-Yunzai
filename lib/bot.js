...
export default class Yunzai extends EventEmitter {
  ...
  wsConnect(req, socket, head) {
    this.wss.handleUpgrade(req, socket, head, conn => {
      ...
      conn.on("message", msg => this.makeLog("debug", ["消息", msg], `${conn.id} => ws://${req.headers.host}${req.url}`))
      conn.sendMsg = msg => {
  ...
  async fsStat(path) { try {
    ...
    this.makeLog("debug", ["获取", path, "状态错误", err])
    return false
  }}
  ...
  async makeMap(dir, map) {
    ...
    map._set = map.set
    map.set = (key, value) => this.setMap(dir, map, key, value)
    map._delete = map.delete
    map.delete = key => this.delMap(dir, map, key)
    return map
  }

  async setMap(dir, map, key, value) {
    ...
    return map._set(key, value)
  }

  async delMap(dir, map, key) {
    await this.rm(`${dir}/${key}`)
    return map._delete(key)
  }
  ...
  async fileType(data, opts = {}) {
    ...
    try {
      if (Buffer.isBuffer(data)) {
        file.url = opts.name || "Buffer"
        file.buffer = data
      } else {
        file.url = data.replace(/^base64:\/\/.*/, "base64://...")
        file.buffer = await this.Buffer(data, opts)
      }
      if (Buffer.isBuffer(file.buffer)) {
        file.type = await fileTypeFromBuffer(file.buffer)
        file.md5 = md5(file.buffer)
        file.name = opts.name || `${Date.now()}.${file.md5.slice(0,8)}.${file.type.ext}`
      } else {
        file.name = opts.name || `${Date.now()}-path.basename(file.buffer)`
      }
    } catch (err) {
  ...
  makeEvent(data) {
    ...
    if (data.bot.adapter?.id)
      data.adapter = data.bot.adapter.id
    for (const i of [data.friend, data.group, data.member]) {
      if (typeof i != "object") continue
      if (!i.makeForwardMsg)
        i.makeForwardMsg = Bot.makeForwardMsg
      if (!i.sendForwardMsg)
        i.sendForwardMsg = msg => Bot.sendForwardMsg(msg => i.sendMsg(msg), msg)
      if (!i.getInfo)
        i.getInfo = () => i
    }
  }
  ...
  pickFriend(user_id) {
    ...
    logger.error(`获取用户对象错误：找不到用户 ${logger.red(user_id)}`)
  }
  ...
  pickGroup(group_id) {
    ...
    logger.error(`获取群对象错误：找不到群 ${logger.red(group_id)}`)
  }
