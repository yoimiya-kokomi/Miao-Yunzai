...
export default class Yunzai extends EventEmitter {
  ...
  async serverEADDRINUSE(err) {
    ...
    await this.sleep((this.server_listen_time += 1) * 1000)
    this.server.listen(cfg.bot.port)
  }

  async serverLoad() {
    this.server_listen_time = 1
    this.server.listen(cfg.bot.port)
  ...
  makeMap(map_set, map_array) {
    const map = new Map(map_array)
    const set = map.set.bind(map)
    map.set = async (key, value) => {
      const ret = set(key, value)
      await map_set(map)
      return ret
    }
    const del = map.delete.bind(map)
    map.delete = async (key, value) => {
      const ret = del(key, value)
      await map_set(map)
      return ret
    }
    return map
  ...
  async setMap(map, db, set, key, value) {
    if (JSON.stringify(map.get(key)) == JSON.stringify(value))
      return map
    const ret = set(key, value)
    try {
      if (value instanceof Map)
        value = { map_array: Array.from(value) }
      await db.put(key, value)
    } catch (err) {
      this.makeLog("error", ["写入", db.location, key, "错误", value, err])
    }
    return ret
  }

  async delMap(db, del, key) {
    const ret = del(key)
    try {
      await db.del(key)
    } catch (err) {
      this.makeLog("error", ["删除", db.location, key, "错误", err])
    }
    return ret
  }
  ...
  async getMap(dir) {
    ...
    for await (const i of db.keys()) try {
      let value = await db.get(i)
      if (typeof value == "object" && value.map_array)
        value = this.makeMap(value => map.set(i, value), value.map_array)
      map.set(i, value)
    } catch (err) {
      this.makeLog("error", ["读取", dir, i, "错误", err])
      await db.del(i)
    }

    const set = map.set.bind(map)
    map.set = (key, value) => this.setMap(map, db, set, key, value)
    const del = map.delete.bind(map)
    map.delete = key => this.delMap(db, del, key)
