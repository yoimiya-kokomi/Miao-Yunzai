...
export default class Yunzai extends EventEmitter {
  ...
  mkdir(dir) { try {
    ...
    if (!this.mkdir(path.dirname(dir))) return false
    fs.mkdirSync(dir)
    return true
  } catch (err) {
    this.makeLog("error", ["创建", dir, "失败", err])
    return false
  }}
  ...
  rmdir(dir) { try {
    ...
    this.makeLog("error", ["删除", dir, "失败", err])
    return false
  }}
  ...
  rm(file) { try {
    ...
    this.makeLog("error", ["删除", file, "失败", err])
    return false
  }}
  ...
  getMap(dir) {
    ...
    for (const i of fs.readdirSync(dir)) {
      const path = `${dir}/${i}`
      try {
        map.set(i, fs.statSync(path).isDirectory() ?
          this.getMap(path) :
