...
export default class Yunzai extends EventEmitter {
  ...
  serverAuth(req) {
    ...
    for (const i in cfg.server.auth) {
      ...
      if (Object.keys(req.query).length)
        msg.query = req.query
      this.makeLog("error", ["HTTP", req.method, "请求", i, "鉴权失败", msg], `${req.sid} <≠ ${req.rid}`, true)
      return false
    }
    req.next?.()
  }
  ...
  serverHandle(req) {
    ...
    for (const i of ["query", "body"])
      if (Object.keys(req[i]).length)
        msg[i] = req[i]
    this.makeLog(quiet?"debug":"mark", ["HTTP", req.method, "请求", msg], `${req.sid} <= ${req.rid}`, true)
    req.next()
  }
  ...
  wsConnect(req, socket, head) {
    ...
    if (!(path in this.wsf)) {
      this.makeLog("error", ["WebSocket 处理器", path, "不存在", msg], `${req.sid} <≠> ${req.rid}`, true)
      socket.write("HTTP/1.1 404 Not Found\r\n\r\n")
      return socket.destroy()
    }

    this.wss.handleUpgrade(req, socket, head, conn => {
      this.makeLog("mark", ["建立连接", msg], `${req.sid} <=> ${req.rid}`, true)
