...
class Puppeteer {
    ...
    async screenshot(name, data = {}, height) {
        ...
        let image = await browser.newPage().then(async (page) => {
            try {
                ...
                do {
                    ...
                    if (--num) await page.evaluate((height) => window.scrollBy(0, height), height)
                } while (num)
        ...
        this.#shoting.pop()
        time = logger.green((Date.now() - time) + "ms")

        if (!image) {
            logger.error(`[图片生成][${name}] `, "截图失败！", time)
            return
        }

        if (++this.#sCount % this.#rNum === 0) this.#cCount++
        if (this.#cCount && !this.#shoting.length) {
            this.#cCount--
            browser.close()
        }
        ...
        logger.mark(`[图片生成][${this.#sCount}次][${name}]`, log, time)
